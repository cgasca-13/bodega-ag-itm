...
export default function BrandModal({ brand, onClose, onSave }: BrandModalProps) {
  const isEditing = brand !== null;
  const [nombre, setNombre] = React.useState(brand?.nombre || '');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEditing) {
        const brandId = brand.idMarca;
        
        if (!brandId) {
          throw new Error('No se encontró el ID de la marca');
        }
        
        response = await fetch(`/api/auth/catalogues/brand/${brandId}`, {
          method: 'PUT',
          headers: {
        ...
        response = await fetch('/api/auth/catalogues/brand/create', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
