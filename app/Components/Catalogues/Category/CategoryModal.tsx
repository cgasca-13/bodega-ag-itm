...
export default function CategoryModal({ category, onClose, onSave }: CategoryModalProps) {
  ...
  const [nombre, setNombre] = React.useState(category?.nombre || '');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        if (!categoryId) {
          throw new Error('No se encontró el ID de la categoría');
        }
        
        response = await fetch(`/api/auth/catalogues/category/${categoryId}`, {
          method: 'PUT',
          headers: {
        ...
        response = await fetch('/api/auth/catalogues/category/create', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
